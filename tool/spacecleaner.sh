...
UpdateSpaceCleaner() {
	CleanSpace=$(df -m "$MNT" | awk -vmnt="$MNT" '
		$6 == mnt { print $4; exit }
		$5 == mnt { print $3; exit }
	')
	# magical number checking hax
	[ "${CleanSpace##*[!0-9]*}" ] && return
...
if [ "$CleanSpace" -gt "$SpaceRequired" ]; then
	exit 0
fi

print "Free space (before): $CleanSpace MB"
...
print "Free space (after): $CleanSpace MB"
